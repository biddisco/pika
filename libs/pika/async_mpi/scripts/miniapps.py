...
def _computeResourcesNeeded(system, nodes, rpn):
    ...
    if rpn != None:
        ...
        if "Max cores per rank" in system:
            resources["cores_per_rank"] = min(resources["cores_per_rank"], system["Max cores per rank"])                
        resources["threads_per_rank"] = system["Threads per core"] * resources["cores_per_rank"]
        resources["timeout"] = resources["total_ranks"] * system["Timeout per rank"]
    return resources
...
# ------------------------------------------------------------------------------
def _add_extra_csv_flags(nodes, rpn, extra_flags):
    if extra_flags is not None and "--pp-info \"" in extra_flags:
        new_flags = f"--pp-info \"rpn, {rpn}, nodes, {nodes}, "
...
class JobText:
    ...
    #       otherwise rpn has to match with the value given at initialization.
...
def chol(
    ...
    extra_flags = _add_extra_csv_flags(nodes, rpn, extra_flags)
