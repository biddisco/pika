    ...
    std::uint64_t count, bool csv, Executor exec)
{
    ...
    high_resolution_timer walltime;
    {
        pika::execution::experimental::lim_debug<4>.debug(
            pika::debug::str<>("FUTURES"));
        pika::execution::experimental::limiting_executor<Executor> limit_exec(
            exec, tasks, tasks + 1000);
        for (uint i = 0; i < count; count++)
        {
            pika::apply(limit_exec, [&]() {
                null_function();
                sanity_check--;
            });
        };
